      ...
      <GlassCard className="p-0 hover:border-indigo-500/30 transition-colors group bg-slate-100 dark:bg-[#1e293b]/80 dark:bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] dark:from-white/5 dark:to-transparent">
        <div className="p-6 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="flex-1 cursor-pointer overflow-hidden" onClick={() => onNavigate(c.id)}>
            <div className="flex flex-wrap items-center gap-3 mb-3">
                <span className={`px-2.5 py-0.5 text-xs rounded-full font-medium border ${
                  ...
                  c.status === 'Ganho' ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-600 dark:text-emerald-300' :
                  c.status === 'Pendente' ? 'border-amber-500/30 bg-amber-500/10 text-amber-600 dark:text-amber-300' :
                  'border-slate-500/30 bg-slate-500/10 text-slate-500 dark:text-slate-300'
                }`}>
                  {c.status}
                </span>
                {c.category && (
                   <span className="px-2 py-0.5 text-xs rounded bg-slate-200 dark:bg-white/5 text-slate-500 dark:text-slate-400 border border-slate-300 dark:border-white/5">
                     {c.category}
                   </span>
                )}
                <span className="text-xs text-slate-500 font-mono tracking-wide hidden sm:inline">{c.cnj}</span>
            </div>
            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-1.5 group-hover:text-indigo-600 dark:group-hover:text-indigo-300 transition-colors truncate" title={c.title}>{c.title}</h3>
          ...
          <div className="flex flex-row lg:flex-col gap-6 lg:gap-2 lg:text-right lg:min-w-[150px] border-t lg:border-t-0 border-slate-200 dark:border-white/5 pt-4 lg:pt-0">
            <div><p className="text-xs text-slate-500 uppercase tracking-wider font-semibold">Valor</p><p className="text-base font-bold text-emerald-600 dark:text-emerald-400">{formatCurrency(c.value)}</p></div>
            <div><p className="text-xs text-slate-500 uppercase tracking-wider font-semibold lg:mt-2">Próx. Audiência</p><p className="text-sm text-slate-700 dark:text-slate-200">{c.nextHearing || '-'}</p></div>
          </div>
          <div className="flex items-center gap-3 lg:border-l border-slate-200 dark:border-white/10 lg:pl-6 relative justify-end">
            <button onClick={() => onNavigate(c.id)} className="p-2.5 rounded-lg bg-slate-200 dark:bg-white/5 hover:bg-indigo-600 hover:text-white text-slate-500 dark:text-slate-400 transition-colors" title="Ver Detalhes"><Eye size={20} /></button>
            <div className="relative">
              <button onClick={() => setOpenActionId(openActionId === c.id ? null : c.id)} className={`p-2.5 rounded-lg transition-colors ${openActionId === c.id ? 'text-slate-900 dark:text-white bg-slate-300 dark:bg-white/10' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-white/5'}`}><MoreHorizontal size={20} /></button>
              <AnimatePresence>
                {openActionId === c.id && (
                  <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} className="absolute right-0 top-full mt-2 w-48 bg-white dark:bg-[#1e293b] border border-slate-200 dark:border-white/10 rounded-xl shadow-2xl z-50 overflow-hidden">
                    <button onClick={() => onEdit(c.id)} className="w-full text-left px-4 py-3 text-sm text-slate-700 dark:text-slate-300 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 hover:text-indigo-600 dark:hover:text-indigo-400 flex items-center gap-2 transition-colors"><Edit2 size={14} /> Editar Processo</button>
                    <button onClick={() => onDelete(c.id)} className="w-full text-left px-4 py-3 text-sm text-slate-700 dark:text-slate-300 hover:bg-rose-50 dark:hover:bg-rose-500/10 hover:text-rose-600 dark:hover:text-rose-400 flex items-center gap-2 transition-colors"><Trash2 size={14} /> Excluir Processo</button>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
  ...
  return (
    <div className="space-y-8 pb-10">
      <div className="flex flex-col md:flex-row justify-between md:items-end gap-4 mb-6">
        <div><h1 className="text-3xl font-bold text-slate-900 dark:text-white">Processos Jurídicos</h1><p className="text-slate-500 dark:text-slate-400 mt-1">Gerencie seus casos, prazos e andamentos.</p></div>
        <div className="flex items-center gap-3 w-full md:w-auto">
          <button onClick={handleExportCSV} className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-white dark:bg-white/5 hover:bg-slate-50 dark:hover:bg-white/10 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-colors text-sm font-medium border border-slate-200 dark:border-white/10">
      ...
      <GlassCard className="p-4 space-y-4">
         <div className="flex flex-col xl:flex-row gap-4">
            <div className="relative flex-1">
              ...
              <input type="text" placeholder="Buscar por número, cliente ou título..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-white dark:bg-slate-900/50 border border-slate-300 dark:border-white/10 rounded-xl py-2.5 pl-10 pr-10 text-slate-900 dark:text-slate-200 focus:outline-none focus:border-indigo-500 transition-colors placeholder:text-slate-400 dark:placeholder:text-slate-600" />
              {searchTerm && <button onClick={() => setSearchTerm('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-800 dark:hover:text-white"><X size={16} /></button>}
            </div>
               ...
               <div className="flex gap-2">
                   ...
                   <div className="relative">
                       <select 
                           ...
                           className="h-full bg-white dark:bg-white/5 border border-slate-300 dark:border-white/10 rounded-lg py-2 pl-3 pr-8 text-sm text-slate-700 dark:text-slate-300 focus:border-indigo-500 outline-none appearance-none cursor-pointer hover:bg-slate-50 dark:hover:bg-white/10 transition-colors"
                       >
                           <option value="updated" className="bg-white dark:bg-slate-800 text-slate-900 dark:text-white">Recentes</option>
                           <option value="created" className="bg-white dark:bg-slate-800 text-slate-900 dark:text-white">Criação</option>
                           <option value="value" className="bg-white dark:bg-slate-800 text-slate-900 dark:text-white">Valor</option>
                       </select>
                       <button 
                           onClick={() => setSortOrder(prev => prev === 'asc' ? 'desc' : 'asc')}
                           className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-800 dark:hover:text-white"
                       >
                           {sortOrder === 'asc' ? <SortAsc size={14} /> : <SortDesc size={14} />}
                       </button>
                   </div>

                   <button onClick={() => setShowFilters(!showFilters)} className={`px-4 py-2.5 rounded-lg border transition-colors flex items-center gap-2 whitespace-nowrap ${showFilters ? 'bg-indigo-600/10 dark:bg-indigo-600/20 border-indigo-500 text-indigo-600 dark:text-indigo-300' : 'bg-white dark:bg-white/5 border-slate-300 dark:border-white/10 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-white/10'}`}>
                       <Filter size={18} /> <span className="hidden sm:inline">Filtros</span>
                   </button>
               </div>
               
               <div className="flex gap-2">
                   <button onClick={() => setShowOnlyMyCases(!showOnlyMyCases)} className={`flex-1 sm:flex-none px-4 py-2.5 rounded-lg border transition-colors flex items-center justify-center gap-2 whitespace-nowrap ${showOnlyMyCases ? 'bg-indigo-500/10 dark:bg-indigo-500/20 border-indigo-500 text-indigo-600 dark:text-indigo-300' : 'bg-white dark:bg-white/5 border-slate-300 dark:border-white/10 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-white/10'}`}><UserCheck size={18} /><span className="hidden sm:inline">Meus</span></button>
                   <div className="flex bg-white dark:bg-white/5 rounded-lg p-1 border border-slate-300 dark:border-white/10 shrink-0">
                     <button onClick={() => setViewMode('list')} className={`p-1.5 rounded-md transition-all ${viewMode === 'list' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-slate-800 dark:hover:text-white'}`}><ListIcon size={20} /></button>
                     <button onClick={() => setViewMode('board')} className={`p-1.5 rounded-md transition-all ${viewMode === 'board' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-slate-800 dark:hover:text-white'}`}><LayoutGrid size={20} /></button>
                   </div>
               </div>
            </div>
         </div>
         ...
         <AnimatePresence>
             {showFilters && (
                 <motion.div 
                    ...
                    className="border-t border-slate-200 dark:border-white/5 pt-4 overflow-hidden"
                 >
                     <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                         <div className="space-y-1">
                             <label className="text-xs text-slate-500 dark:text-slate-400 font-medium ml-1">Categoria</label>
                             <div className="relative">
                                 <select 
                         ...
                         <div className="space-y-1">
                             <label className="text-xs text-slate-500 dark:text-slate-400 font-medium ml-1">Atualizado entre</label>
                             <div className="flex gap-2">
                                 <input type="date" value={dateRange.start} onChange={(e) => setDateRange({...dateRange, start: e.target.value})} className="flex-1 bg-white dark:bg-slate-900/50 border border-slate-300 dark:border-white/10 rounded-lg py-2 px-3 text-sm text-slate-900 dark:text-slate-200 outline-none dark:scheme-dark" />
                                 <input type="date" value={dateRange.end} onChange={(e) => setDateRange({...dateRange, end: e.target.value})} className="flex-1 bg-white dark:bg-slate-900/50 border border-slate-300 dark:border-white/10 rounded-lg py-2 px-3 text-sm text-slate-900 dark:text-slate-200 outline-none dark:scheme-dark" />
                             </div>
                         </div>
                         <div className="flex gap-2">
                             ...
                             <button onClick={clearFilters} className="px-3 bg-white dark:bg-white/5 hover:bg-slate-100 dark:hover:bg-white/10 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white py-2 rounded-lg text-sm transition-colors border border-slate-200 dark:border-transparent" title="Limpar Filtros"><X size={16} /></button>
                         </div>
                     </div>
                 </motion.div>
             )}
      ...
      {viewMode === 'list' ? (
          <div className="space-y-4 min-h-[400px]">
            ...
            {!loading && cases.length === 0 && (<div className="flex flex-col items-center justify-center py-20 text-slate-500"><Briefcase size={32} className="opacity-50 mb-4" /><p className="text-lg font-medium text-slate-400 dark:text-slate-300">Nenhum processo encontrado</p></div>)}
          </div>
      ) : (
              ...
              <div className="flex gap-6 min-w-[1000px] h-full">
                  {[CaseStatus.ACTIVE, CaseStatus.PENDING, CaseStatus.WON, CaseStatus.ARCHIVED].map(status => (
                      <div key={status} className="flex-1 min-w-[300px] flex flex-col h-full">
                          <div className={`flex items-center justify-between mb-4 px-3 py-2 rounded-lg border bg-white dark:bg-white/5 border-slate-200 dark:border-white/10`}>
                              <h3 className="font-bold text-slate-800 dark:text-white">{status}</h3>
                              <span className="text-xs font-medium px-2 py-0.5 rounded bg-slate-100 dark:bg-black/20 text-slate-600 dark:text-slate-400">{cases.filter(c => c.status === status).length}{hasMore && "+"}</span>
                          </div>
      ...
      {/* Quick Client Modal */}
      <Modal isOpen={isQuickClientModalOpen} onClose={() => setIsQuickClientModalOpen(false)} title="Cadastro Rápido de Cliente" footer={<><button disabled={submittingClient} onClick={() => setIsQuickClientModalOpen(false)} className="px-3 py-1.5 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white">Cancelar</button><button disabled={submittingClient} onClick={handleQuickClientSave} className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg flex items-center gap-2">{submittingClient ? <Loader2 size={12} className="animate-spin"/> : 'Salvar'}</button></>}>
         <div className="space-y-3"><input type="text" placeholder="Nome Completo *" value={quickClientData.name} onChange={e => setQuickClientData({...quickClientData, name: e.target.value})} className="w-full bg-white dark:bg-white/5 border border-slate-300 dark:border-white/10 rounded-lg p-2 text-slate-900 dark:text-white outline-none focus:border-emerald-500" /><div className="flex gap-2"><button type="button" onClick={() => setQuickClientData({...quickClientData, type: 'PF', doc: ''})} className={`flex-1 py-1 text-xs rounded border transition-colors ${quickClientData.type === 'PF' ? 'border-emerald-500 text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10' : 'border-slate-300 dark:border-white/10 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-white/5'}`}>Pessoa Física</button><button type="button" onClick={() => setQuickClientData({...quickClientData, type: 'PJ', doc: ''})} className={`flex-1 py-1 text-xs rounded border transition-colors ${quickClientData.type === 'PJ' ? 'border-emerald-500 text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10' : 'border-slate-300 dark:border-white/10 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-white/5'}`}>Pessoa Jurídica</button></div><input type="text" placeholder={quickClientData.type === 'PF' ? 'CPF' : 'CNPJ'} value={quickClientData.doc} onChange={e => { const masked = quickClientData.type === 'PF' ? masks.cpf(e.target.value) : masks.cnpj(e.target.value); setQuickClientData({...quickClientData, doc: masked}); }} maxLength={quickClientData.type === 'PF' ? 14 : 18} className="w-full bg-white dark:bg-white/5 border border-slate-300 dark:border-white/10 rounded-lg p-2 text-slate-900 dark:text-white outline-none focus:border-emerald-500" /></div>
      </Modal>

      <Modal isOpen={!!caseToDelete} onClose={() => setCaseToDelete(null)} title="Confirmar Exclusão" footer={<div className="flex gap-3 justify-end"><button onClick={() => setCaseToDelete(null)} className="px-4 py-2 text-slate-500 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white">Cancelar</button><button onClick={confirmDelete} className="px-4 py-2 bg-rose-600 hover:bg-rose-500 text-white rounded-lg">Excluir</button></div>}>
         <p className="text-center text-slate-700 dark:text-slate-300">Ação irreversível. O processo e todo seu histórico serão apagados permanentemente. Deseja continuar?</p>
      </Modal>
    </div>
  );
};
