    ...
    >
      <div className="flex justify-between items-center mb-2 px-4 pt-4 shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="bg-white/5 text-slate-400 text-xs px-2 py-0.5 rounded-full border border-white/5 font-mono">{tasks.length}</span>
        </div>
      ...
      <div className="space-y-3 flex-1 overflow-y-auto custom-scrollbar p-3">
        ...
        {tasks.length === 0 && (
          <div className="h-32 border-2 border-dashed border-white/5 rounded-xl flex flex-col items-center justify-center text-slate-600 text-sm bg-white/5 gap-2 group hover:border-white/10 transition-colors cursor-pointer" onClick={onAddTask}>
            <Plus size={24} className="opacity-50 group-hover:scale-110 transition-transform" />
      ...
      <Modal
        ...
        footer={
          <div className="flex justify-between w-full items-center">
             <div>
               {editingTask && (
                 <button type="button" onClick={handleDeleteTask} className="px-3 py-2 text-rose-400 hover:bg-rose-500/10 rounded-lg transition-colors flex items-center gap-2 text-sm font-medium">
                   <Trash2 size={16} /> Excluir
                 </button>
               )}
             </div>
             <div className="flex gap-3">
               <button type="button" onClick={() => setIsModalOpen(false)} className="px-4 py-2 rounded-lg text-slate-400 hover:text-white hover:bg-white/5 transition-colors text-sm">Cancelar</button>
      ...
      >
        <form className="space-y-6">
           ...
           <div className="space-y-4">
              <div className="space-y-1.5">
                  ...
                  <input 
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-slate-200 focus:border-indigo-500 focus:outline-none transition-all placeholder:text-slate-600" 
               ...
               <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                 ...
                 <div className="space-y-1.5">
                    ...
                    <select 
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-lg p-2.5 text-slate-200 focus:border-indigo-500 focus:outline-none scheme-dark cursor-pointer"
                    >
           ...
           <div className="bg-slate-900/50 border border-white/10 rounded-xl p-5 space-y-4">
               <div className="flex items-center justify-between">
                   <label className="text-sm font-semibold text-slate-200 flex items-center gap-2">
                      <LinkIcon size={16} className="text-indigo-400" /> Vínculo (Opcional)
                   </label>
                   {formData.linkType !== 'none' && (
                       <button type="button" onClick={() => setFormData({...formData, linkType: 'none'})} className="text-xs text-rose-400 hover:text-rose-300 transition-colors flex items-center gap-1">
                           <Trash2 size={10} /> Remover
                       </button>
                   )}
               </div>
               
               <div className="grid grid-cols-3 gap-3 p-1 bg-black/20 rounded-lg">
                  <button 
                     type="button"
                     onClick={() => setFormData({...formData, linkType: 'none', creationMode: 'existing'})}
                     className={`py-2 px-3 text-xs font-medium rounded-md transition-all ${formData.linkType === 'none' ? 'bg-indigo-600 text-white shadow' : 'text-slate-400 hover:text-white'}`}
                  >
                     Nenhum
                  </button>
                  <button 
                     ...
                     className={`py-2 px-3 text-xs font-medium rounded-md transition-all ${formData.linkType === 'case' ? 'bg-indigo-600 text-white shadow' : 'text-slate-400 hover:text-white'}`}
                  >
                     Processo
                  </button>
                  <button 
                     ...
                     className={`py-2 px-3 text-xs font-medium rounded-md transition-all ${formData.linkType === 'client' ? 'bg-indigo-600 text-white shadow' : 'text-slate-400 hover:text-white'}`}
                  >
                     Cliente
                  </button>
               </div>
               ...
               {formData.linkType !== 'none' && (
                  <div className="animate-fade-in pt-2">
                     <div className="flex gap-6 mb-4 text-xs border-b border-white/5 pb-2">
                        <label className={`flex items-center gap-2 cursor-pointer transition-colors ${formData.creationMode === 'existing' ? 'text-indigo-400 font-bold' : 'text-slate-500 hover:text-white'}`}>
                           <input 
                              type="radio" 
                        ...
                        <label className={`flex items-center gap-2 cursor-pointer transition-colors ${formData.creationMode === 'new' ? 'text-indigo-400 font-bold' : 'text-slate-500 hover:text-white'}`}>
                           <input 
                              type="radio" 
                        ...
                        formData.creationMode === 'existing' ? (
                           <div className="relative">
                              ...
                              <select 
                                ...
                                className="w-full bg-slate-900 border border-white/10 rounded-lg py-2.5 pl-9 pr-4 text-slate-200 text-sm focus:border-indigo-500 outline-none"
                              >
                           ...
                           <div className="grid grid-cols-1 md:grid-cols-2 gap-3 animate-fade-in">
                              <div className="space-y-1">
                                <label className="text-[10px] text-slate-400 uppercase">Título do Processo</label>
                                <input 
                                   type="text" 
                                   placeholder="Ex: Ação Trabalhista"
                                   value={formData.newEntityName}
                                   onChange={(e) => setFormData({...formData, newEntityName: e.target.value})}
                                   className="w-full bg-slate-900 border border-white/10 rounded-lg p-2.5 text-slate-200 text-sm focus:border-indigo-500 outline-none"
                                />
                              </div>
                              <div className="space-y-1">
                                <label className="text-[10px] text-slate-400 uppercase">Cliente do Processo</label>
                                <select 
                     ...
                     {formData.linkType === 'client' && (
                        formData.creationMode === 'existing' ? (
                           ...
                           <div className="space-y-1 animate-fade-in">
                              <label className="text-[10px] text-slate-400 uppercase">Nome Completo / Razão Social</label>
                              <input 
                                 type="text" 
           ...
           <div className="space-y-4">
               ...
               <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-slate-400">Descrição / Observações</label>
                  <textarea 
                    rows={3} 
                    value={formData.description} 
                    onChange={(e) => setFormData({...formData, description: e.target.value})}
                    className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-slate-200 focus:border-indigo-500 focus:outline-none resize-none transition-all placeholder:text-slate-600 leading-relaxed"
